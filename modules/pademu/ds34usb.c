...
static void readReport(u8 *data, int pad_idx)
{
    ...
    if (pad->type == DF) {
        ...
        /*
        if (report->brakePedal == 0 &&
            report->gasPedal == 0 &&
            report->wheel == 0) // ledrumble cmd causes null report sometime
        {
            return;
        }
        */
        
        translate_wheel_negcon(report, &pad->ds2);        
    ...
    if (data[0]) {
        ...
        if (pad->type == DS3) {
            ...
            //padMacroPerform(&pad->ds2, report->PSButton);
            if (report->L3) {                                    // display battery level
                if (report->R3 && (pad->btn_delay == MAX_DELAY)) { // PSButton) PS + SELECT
                    if (pad->analog_btn < 2)                           // unlocked mode
                        pad->analog_btn = !pad->analog_btn;
