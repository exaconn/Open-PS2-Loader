...
static void usb_config_set(int result, int count, void *arg)
{
    ...
    if (ds34pad[pad].type == DF) { 
        usb_buf[0] = 0x4E; // default spring
        usb_buf[1] = 0x00;
        usb_buf[2] = 0x00;
        usb_buf[3] = 0x00;
        usb_buf[4] = 0xFF; // strong
        usb_buf[5] = 0x00;
        usb_buf[6] = 0x00;
...
static void readReport(u8 *data, int pad)
{
    if (ds34pad[pad].type == DF) {
        ...
        ds34pad[pad].data[0] = ~(report->start << 3 | up << 4 | right << 5 | down << 6 | left << 7);
        ds34pad[pad].data[1] = ~(report->R1 << 3 | report->triangle << 4 | report->circle << 5);
           
        ds34pad[pad].data[2] = report->wheel >> 2; // 8bit conversion
        ds34pad[pad].data[3] = 255 - report->gasPedal; // inverted
        ds34pad[pad].data[4] = 255 - report->brakePedal; // inverted
        ds34pad[pad].data[5] = report->L1 * 255;
    ...
    if (ds34pad[pad].type == DF) {
        ...
        ds34pad[pad].data[2] = 0x7F;
        ds34pad[pad].data[3] = 0x7F;
        ds34pad[pad].data[4] = report->wheel >> 2; // 8bit conversion
        ds34pad[pad].data[5] = 0x7F;
        ...
        ds34pad[pad].data[12] = 255 - report->gasPedal; // cross inverted
        ds34pad[pad].data[13] = 255 - report->brakePedal; // square inverted
